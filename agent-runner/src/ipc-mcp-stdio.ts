...
interface IpcPollResult {
  status: string;
  result?: string;
  error?: string;
}

async function pollForIpcResult(
  resultFile: string,
  timeoutMs: number,
  pollMs = 500,
): Promise<IpcPollResult | null> {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    try {
      const raw = fs.readFileSync(resultFile, 'utf-8');
      try { fs.unlinkSync(resultFile); } catch { /* best-effort cleanup */ }
      return JSON.parse(raw) as IpcPollResult;
    } catch (err: unknown) {
...
server.tool(
  ...
  {},
  async () => {
    const indexFile = path.join(IPC_DIR, 'workflows', 'index.json');
    ...
    try {
      ...
      const workflows = JSON.parse(fs.readFileSync(indexFile, 'utf-8'));
...
server.tool(
  ...
  async (args) => {
    ...
    const resultFile = path.join(IPC_DIR, 'worker-results', `${delegationId}.json`);
    const result = await pollForIpcResult(resultFile, 300_000);
    if (!result) {
    ...
    return { content: [{ type: 'text' as const, text: result.result ?? 'No response' }] };
  },
);
...
server.tool(
  ...
  async (args) => {
    ...
    const resultFile = path.join(IPC_DIR, 'agent-results', `${requestId}.json`);
    const result = await pollForIpcResult(resultFile, 300_000);
    if (!result) {
      return { content: [{ type: 'text' as const, text: `send_to_agent timed out after 5 minutes (target: ${args.target_agent})` }], isError: true };
    }
    if (result.status === 'error') {
      return { content: [{ type: 'text' as const, text: `Agent error: ${result.error}` }], isError: true };
    }
    return { content: [{ type: 'text' as const, text: result.result ?? 'No response' }] };
  },
);
...
server.tool(
  ...
  async (args) => {
    ...
    const resultFile = path.join(IPC_DIR, 'email-results', `${requestId}.json`);
    const result = await pollForIpcResult(resultFile, 60_000);
    if (!result) {
