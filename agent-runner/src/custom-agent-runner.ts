 ...
 * Uses cambot-llm package for providers, tools, executor, and memory.
 */
import fs from 'fs';
...
import type { ProviderConfig, ProviderName } from 'cambot-llm';
    ...
    try {
      ...
      for (const tool of mcpTools) {
        if (wantAllMcp || requestedTools.has(tool.name)) {
          ...
          toolRegistry.register(tool.name, tool, (args: Record<string, unknown>) => bridge.callTool(tool.name, args));
        }
      }
    } catch (err) {
