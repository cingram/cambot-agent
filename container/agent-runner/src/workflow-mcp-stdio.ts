...
server.tool(
  'get_workflow',
  'Get the full workflow definition including all steps, policy, and schedule. Reads from the local snapshot — no IPC round-trip needed.',
  {
    workflow_id: z.string().describe('The workflow ID to retrieve'),
  },
  async (args) => {
    // Read from enhanced snapshot
    const snapshotFile = path.join(IPC_DIR, 'current_workflows.json');
    if (!fs.existsSync(snapshotFile)) {
      return {
        content: [{ type: 'text' as const, text: 'No workflows available.' }],
        isError: true,
      };
    }

    const workflows = JSON.parse(fs.readFileSync(snapshotFile, 'utf-8')) as Array<Record<string, unknown>>;
    const workflow = workflows.find(w => w.id === args.workflow_id);
