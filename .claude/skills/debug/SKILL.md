...
# CamBot-Agent Container Debugging
...
─────────────────────────────────────────────────────────────
src/container-runner.ts               agent-runner/
    │                                      │
...
|-----|----------|---------|
| **Main app logs** | `logs/cambot-agent.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/cambot-agent.error.log` | Host-side errors |
...
```
**Fix:** Ensure `.env` file exists with an API key:
```bash
cat .env  # Should show:
...
**Workaround:** The system extracts only authentication variables (`ANTHROPIC_API_KEY`) from `.env` and passes them to the container via stdin. Other env vars are not exposed.

To verify env vars are reaching the container:
```bash
echo '{}' | docker run -i \
  --entrypoint /bin/bash cambot-agent-agent:latest \
  -c 'echo "API key is passed via stdin secrets, not env vars"'
```
...
```bash
docker run --rm --entrypoint /bin/bash cambot-agent-agent:latest -c 'ls -la /workspace/'
```

Expected structure:
```
/workspace/
├── env-dir/env           # (Legacy — secrets now passed via stdin)
...
```bash
docker run --rm --entrypoint /bin/bash cambot-agent-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash cambot-agent-agent:latest
```
...
# Also clear the session ID from CamBot-Agent's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/cambot-agent.log | tail -5
...
```bash
echo "=== Checking CamBot-Agent Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo cambot-agent-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/cambot-agent.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
