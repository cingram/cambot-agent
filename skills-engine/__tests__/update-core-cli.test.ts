...
import { cleanup, createTempDir, initGitRepo, setupCambotAgentDir } from './test-helpers.js';
...
describe('update-core.ts CLI flags', () => {
  ...
  beforeEach(() => {
    ...
    // Write state file
    const statePath = path.join(tmpDir, '.cambot-agent', 'state.yaml');
    fs.writeFileSync(
      statePath,
      stringify({
  ...
  it('--json --preview-only outputs JSON preview without applying', () => {
    const baseDir = path.join(tmpDir, '.cambot-agent', 'base');
