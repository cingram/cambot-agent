...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.cambot-agent')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.cambot-agent'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active cambot-agent`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('cambot-agent')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'cambot-agent', 'mount-allowlist.json'))) {
