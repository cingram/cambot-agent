...
if [ "$PLATFORM" = "macos" ]; then
  echo "  Next steps:"
  echo "    1. Edit $AGENT_DIR/.env with your API keys (if not done)"
  ...
  echo "    1. Edit $AGENT_DIR/.env with your API keys (if not done)"
  echo "    2. Start the service:"
  echo "       sudo systemctl start cambot-agent"
  echo "       sudo systemctl start cambot-ui"
  echo ""
  echo "    Enable auto-start:"
  echo "       sudo systemctl enable cambot-agent cambot-ui"
  echo ""
  echo "    Logs:"
  echo "       journalctl -u cambot-agent -f"
fi
echo ""
