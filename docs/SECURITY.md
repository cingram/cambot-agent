...
**External Allowlist** - Mount permissions stored at `~/.config/cambot-agent/mount-allowlist.json`, which is:
...
```typescript
const allowedVars = ['ANTHROPIC_API_KEY', 'OPENAI_API_KEY', 'XAI_API_KEY', 'GOOGLE_API_KEY'];
```
