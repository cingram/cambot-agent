...
describe('cleanupOrphans', () => {
  it('kills orphaned cambot-agent containers and verifies', () => {
    // docker ps for agent containers returns names
    ...
    mockExecSync.mockReturnValueOnce('');
    // docker ps for worker containers returns empty
    mockExecSync.mockReturnValueOnce('');

    cleanupOrphans();

    // ps(agents) + 2 kills + verification ps + ps(workers) = 5
  ...
  it('does nothing when no orphans exist', () => {
    // docker ps for agent containers returns empty
    mockExecSync.mockReturnValueOnce('');
    // docker ps for worker containers returns empty
    mockExecSync.mockReturnValueOnce('');
